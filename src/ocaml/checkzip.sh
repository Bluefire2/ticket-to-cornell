...
if [ -e final_project.zip ]; then
  echo "final_project.zip exists.  Good."
  if zipinfo -1 final_project.zip | grep -q "^state.ml$"; then
  ...
  echo "final_project.zip does NOT exist.  This is bad.  Have you run \"make zip\"?"
  zip=bad
fi
